...
let audioCtx=null,bgmTimer=null,soundOn=true,bgmGain=null;
...
function ensureAudio(){
 ...
 if(!audioCtx){audioCtx=new Audio();bgmGain=audioCtx.createGain();bgmGain.gain.value=.055;bgmGain.connect(audioCtx.destination);}
...
function tone(frequency,duration=.16,volume=.08,delay=0,type='sine',output=null){
 ...
 osc.connect(gain);gain.connect(output||ctx.destination);osc.start(startAt);osc.stop(startAt+duration+.03);
}
function bgmPhrase(){if(!soundOn||!playing||document.hidden)return;[261.63,329.63,392,329.63].forEach((note,i)=>tone(note,.42,.5,i*.48,'sine',bgmGain));}
function startBgm(){if(!ensureAudio()||bgmTimer)return;bgmPhrase();bgmTimer=setInterval(bgmPhrase,2400);}
...
function toggleSound(){soundOn=!soundOn;updateSoundButton();if(soundOn){ensureAudio();if(playing)startBgm();tone(659.25,.14,.08);}else{stopBgm();if(audioCtx)audioCtx.suspend();}}
...
function start(){stopVoice();clearTimeout(nextTimer);max=Number($('range').value);min=max===9?0:1;score=0;playing=true;progress();renderNumbers();spawn();startBgm();$('helper').textContent=Recognition?'マイクを オンにすると、つづけて こたえられるよ':'このブラウザでは タッチで あそべるよ';}
...
document.addEventListener('visibilitychange',()=>{if(document.hidden){stopVoice();stopBgm();$('helper').textContent='マイクは オフ。オンで さいかいできるよ';}else if(playing&&soundOn)startBgm();});
